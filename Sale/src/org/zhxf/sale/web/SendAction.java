...
public class SendAction extends BaseAction implements ModelDriven<Send> {
	...
	public String insert() throws Exception {
		...
		if(model.getSendQty()>cpy.getStockQty()) {
			return "error";
		}
...
//		Cpy cpy = cpyService.get(Cpy.class, model.getId().getItemNo());
		...
		if(cpy.getStockQty()!=0L) {
			num1 = cpy.getStockQty();
		}
		cpy.setStockQty(num1-model.getSendQty());
		...
		if(storeStockInfo.getQuality()!=0L) {
			num2 = storeStockInfo.getQuality();
		}
		storeStockInfo.setQuality(model.getSendQty()+num2);
